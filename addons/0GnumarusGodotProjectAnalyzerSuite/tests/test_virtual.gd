...
## Virtual-type suite: tuples, structs and aliases are annotation-only
## types — they refine Array/Dictionary/Variant through @var/@param/
## @return but can never appear as declared GDScript types (vartypes,
## `->` arrows, base classes). Interfaces stay lenient (documented).
...
func _v_vartype(h) -> void:
	...
	h.check(_clean(h.analyze_text("extends Node\n# @interface VvFace\n# func:f:void\n# @endinterface\nvar v: VvFace\n", "res://tests/tmp_vv_t07.gd")), "interface vartype stays lenient")
