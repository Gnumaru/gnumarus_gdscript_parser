...
func _r_tabs(h) -> void:
	...
	h.check(tree.columns == 9, "files tree columns")
	var dtree: Tree = (d as Object).get("_dirs_tree")
	h.check(dtree.columns == 9, "dirs tree columns")
...
func _r_dirs_tree(h) -> void:
	...
	h.check(ftree.get_root().get_child(0).get_child(0).get_text(0) == "res://a.gd", "dirs sort spares files tree")
...
func _r_column_collapse(h) -> void:
	...
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, tree)
	...
	d.call("_on_column_title_clicked", 0, MOUSE_BUTTON_RIGHT, tree)
	...
	d.call("_on_column_title_clicked", 99, MOUSE_BUTTON_LEFT, tree)
	...
	h.check(d2.call("_column_fit_width", tree2, 5) >= maxi(size_w, size_title_w), "fit covers cells and title")
	h.check(d2.call("_column_fit_width", tree2, -1) == 0, "fit rejects negative")
	h.check(d2.call("_column_fit_width", tree2, 9) == 0, "fit rejects overflow")
	...
	d2.call("_on_column_title_clicked", 5, MOUSE_BUTTON_LEFT, tree2)
	d2.call("set_census", {
...
func _r_flat_view(h) -> void:
	...
	h.check(vtabs.get_tab_title(1) == Dock.TAB_TEXT, "text view second")
	_flip_panel(d, Dock.PANEL_FILES, "sort", 1)
	h.check(flat.get_root().get_child(0).get_child(0).get_text(0) == Dock.flat_file_row(files[0]), "flat view follows files sort")
	_flip_panel(d, Dock.PANEL_FILES, "addons", 0)
