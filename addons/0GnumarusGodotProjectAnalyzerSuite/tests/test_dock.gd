...
func _r_tabs(h) -> void:
	...
	h.check(tree.get_column_title(0) == "Path", "files first column path")
	h.check(tree.get_column_title(3) == "Modified", "files last column modified")
...
func _r_file_tree(h) -> void:
	...
	h.check(sub.collapsed, "subgroups start collapsed")
...
func _r_column_collapse(h) -> void:
	...
	h.check(bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(1, false)), "click records collapse")
	...
	h.check(dtree.is_column_expanding(1), "click spares sibling tree")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, dtree, Dock.PANEL_DIRS)
	h.check(not dtree.is_column_expanding(1), "sibling collapse independent")
	h.check(not tree.is_column_expanding(1), "first collapse kept")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(not bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(1, true)), "second click records restore")
	h.check(tree.is_column_expanding(1) and not tree.is_column_clipping_content(1), "second click restores tree column")
	h.check(not dtree.is_column_expanding(1), "restore spares sibling collapse")
	d.call("_on_column_title_clicked", 0, MOUSE_BUTTON_RIGHT, tree, Dock.PANEL_FILES)
	h.check(not bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(0, false)), "right click ignored")
	h.check(tree.is_column_expanding(0), "right click spares tree column")
	d.call("_on_column_title_clicked", 99, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).size() == 1, "out-of-range click ignored")
	d.free()
	var d2 := _new_dock()
	...
	d2.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, tree2, Dock.PANEL_FILES)
	d2.call("set_census", {
	...
	h.check(not tree2.is_column_expanding(1), "rebuild keeps collapse")
	d2.free()
