...
## Flips one Files-tab panel control: "addons" via pressed, "desc"
## via toggled, "sort" via item_selected. Never fails.
func _flip_panel(d: Control, panel: String, key: String, to: int) -> void:
	var w: Control = ((d as Object).get("_panel_widgets") as Dictionary).get(panel, {}).get(key, null)
	if key == "sort" and w is OptionButton:
		(w as OptionButton).select(to)
		(w as OptionButton).emit_signal("item_selected", to)
	elif key == "desc" and w is CheckButton:
		(w as CheckButton).button_pressed = to == 1
		(w as CheckButton).emit_signal("toggled", to == 1)
	elif w is Button:
		(w as Button).button_pressed = to == 1
		(w as Button).pressed.emit()


func _panel_widget(d: Control, panel: String, key: String) -> Control:
	return ((d as Object).get("_panel_widgets") as Dictionary).get(panel, {}).get(key, null)


func _panel_state(d: Control, panel: String) -> Dictionary:
	return ((d as Object).get("_panel") as Dictionary).get(panel, {})
...
func _r_census(h) -> void:
	...
	_flip_panel(d, Dock.PANEL_FILES, "addons", 0)
	...
	_flip_panel(d, Dock.PANEL_FILES, "addons", 1)
	...
	h.check(not bool(_panel_state(d, Dock.PANEL_FILES).get("include_addons", true)), "apply migrates flat addons into files panel")
	h.check(not bool(_panel_state(d, Dock.PANEL_DIRS).get("include_addons", true)), "apply migrates flat addons into dirs panel")
	h.check(not ((_panel_widget(d, Dock.PANEL_FILES, "addons") as Button).button_pressed), "apply syncs files addons button")
...
func _r_tabs(h) -> void:
	...
	h.check(vtabs.tabs_visible, "display tabs shown")
	h.check(vtabs.get_tab_count() == 2, "two display views")
	h.check(vtabs.get_tab_title(0) == Dock.TAB_TABLE, "table view first")
	h.check(vtabs.get_tab_title(1) == Dock.TAB_TEXT, "text view second")
	var tree: Tree = (d as Object).get("_files_tree")
	h.check(tree.columns == Dock.FILES_COLUMNS, "files tree four columns")
	h.check(tree.get_column_title(0) == "Path" + Dock.SORT_ASC_MARK, "sort column marked ascending")
	h.check(tree.get_column_title(3) == "Modified", "unsorted column plain")
	var dtree: Tree = (d as Object).get("_dirs_tree")
	h.check(dtree.columns == 9, "dirs tree keeps nine columns")
	h.check((d as Object).get("_files_flat").columns == 1, "files text single column")
	h.check((d as Object).get("_dirs_flat").columns == 1, "dirs text single column")
	...
	h.check(dtree.get_root().get_child_count() == 1, "dirs hint when uncensused")
	d.call("set_census", mixed)
	...
	h.check(dtree.get_root().get_child_count() == 1, "legacy dirs hint")
	h.check(dtree.get_root().get_child(0).get_text(0) == Dock.CENSUS_HINT, "legacy dirs hint text")
...
func _r_file_tree(h) -> void:
	...
	h.check(Dock.ext_label("gd") == ".gd", "ext label dots")
	h.check(Dock.ext_label("(no ext)") == "(no ext)", "ext label keeps bare bucket")
	var grouped := Dock.group_files_by_ext([{"path": "res://b.tscn"}, {"path": "res://a.gd"}, "junk", {"path": "res://c.gd"}])
	h.check((grouped as Array).map(func(g: Variant) -> String: return str((g as Array)[0])) == ["gd", "tscn"], "ext groups alphabetical")
	h.check(_paths((grouped[0] as Array)[1]) == ["res://a.gd", "res://c.gd"], "ext groups keep input order")
	var st := Dock.ext_bucket_stats([
		{"path": "res://a.gd", "size": 300, "created": 1000, "modified": 2000},
		{"path": "res://b.gd", "size": 100, "created": 0, "modified": 1500},
		"junk",
	])
	h.check(int(st.get("count", 0)) == 2, "bucket counts dicts")
	h.check(int(st.get("size_sum", 0)) == 400, "bucket sums bytes")
	h.check(int(st.get("size_min", 0)) == 100 and int(st.get("size_max", 0)) == 300 and int(st.get("size_avg", 0)) == 200, "bucket size min/max/avg")
	h.check(int(st.get("created_min", -1)) == 1000 and int(st.get("created_max", -1)) == 1000, "bucket skips unknown stamps")
	h.check(int(st.get("modified_min", -1)) == 1500 and int(st.get("modified_max", -1)) == 2000, "bucket tracks known stamps")
	h.check(Dock.ext_bucket_stats([]) == {"count": 0, "size_sum": 0, "size_min": 0, "size_max": 0, "size_avg": 0, "created_min": 0, "created_max": 0, "modified_min": 0, "modified_max": 0}, "bucket empty zeros")
	h.check(Dock.census_stamp_or_q(1000) == "1970-01-01 00:16:40", "stamp renders known")
	h.check(Dock.census_stamp_or_q(0) == "?", "stamp questions unknown")
	var census := {
	...
	var tree: Tree = (d as Object).get("_files_tree")
	h.check(tree.get_root().get_child_count() == 2, "file partition groups")
	...
	h.check(not proj.collapsed, "groups start expanded")
	h.check(proj.get_child_count() == 1, "project ext subgroup")
	var sub := proj.get_child(0)
	h.check(sub.get_text(0) == ".gd", "subgroup dotted label")
	h.check(not sub.collapsed, "subgroups start expanded")
	h.check(sub.get_child_count() == 2, "subgroup file rows")
	h.check(sub.get_child(0).get_text(0) == "res://a.gd", "default sort path")
	h.check(sub.get_text(1) == "400 B", "subgroup sums bytes")
	h.check(sub.get_tooltip_text(1) == "min: 100 B, max: 300 B, avg: 200 B", "subgroup size min/max/avg tooltip")
	h.check(sub.get_text(2) == "", "subgroup unknown creation blank")
	h.check(sub.get_tooltip_text(2) == "min: ?, max: ?", "subgroup creation min/max tooltip")
	h.check(sub.get_text(3) == "1970-01-01 00:33:20", "subgroup newest modification")
	h.check(sub.get_tooltip_text(3) == "min: 1970-01-01 00:16:40, max: 1970-01-01 00:33:20", "subgroup modification min/max tooltip")
	h.check(sub.get_child(0).get_text(1) == "300 B", "file size human")
	h.check(sub.get_child(0).get_text(2) == "", "file unknown creation blank")
	h.check(sub.get_child(0).get_text(3) == "1970-01-01 00:16:40", "file modified stamp")
	var md: Dictionary = sub.get_child(0).get_metadata(0)
	h.check(str(md.get("path", "")) == "res://a.gd" and int(md.get("line", 0)) == 4, "file row targets first issue")
	var bmd: Dictionary = sub.get_child(1).get_metadata(0)
	h.check(int(bmd.get("line", 0)) == 1, "clean file targets line one")
	var dtree: Tree = (d as Object).get("_dirs_tree")
	h.check(dtree.get_root().get_child_count() == 1, "dirs tree skips file groups")
	_goto_seen.clear()
	proj.collapsed = false
	sub.collapsed = false
	sub.get_child(0).select(0)
	h.check(_goto_seen.size() == 1 and str((_goto_seen[0] as Dictionary).get("path", "")) == "res://a.gd", "file row navigates")
	sub.select(0)
	h.check(_goto_seen.size() == 1, "subgroup row inert")
	_flip_panel(d, Dock.PANEL_FILES, "sort", 1)
	proj = tree.get_root().get_child(0)
	h.check(proj.get_child(0).get_child(0).get_text(0) == "res://b.gd", "sort dropdown re-sorts")
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "size", "sort dropdown stores key")
	_flip_panel(d, Dock.PANEL_FILES, "desc", 1)
	proj = tree.get_root().get_child(0)
	h.check(proj.get_child(0).get_child(0).get_text(0) == "res://a.gd", "desc toggle inverts")
	h.check(bool(_panel_state(d, Dock.PANEL_FILES).get("descending", false)), "desc toggle stores order")
	...
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "path", "apply guards sort key")
	h.check(not bool(_panel_state(d, Dock.PANEL_FILES).get("descending", false)), "apply guards descending")
	h.check((_panel_widget(d, Dock.PANEL_FILES, "sort") as OptionButton).selected == 0, "apply syncs sort dropdown")
	d.call("set_census", {
		"extensions": {"gd": 1, "tscn": 1}, "total": 2,
		"project": {"extensions": {"gd": 1, "tscn": 1}, "total": 2},
		"addons": {"extensions": {}, "total": 0},
		"files": [
			{"path": "res://b.tscn", "size": 50, "created": 0, "modified": 500},
			{"path": "res://a.gd", "size": 300, "created": 0, "modified": 1000},
		],
		"dirs": [],
	})
	proj = tree.get_root().get_child(0)
	var subs: Array = []
	for i in range(proj.get_child_count()):
		subs.append(proj.get_child(i).get_text(0))
	h.check(subs == [".gd", ".tscn"], "partition groups files by extension")
	h.check(proj.get_child(0).get_child(0).get_text(0) == "res://a.gd", "gd subgroup holds gd files")
	h.check(proj.get_child(1).get_child(0).get_text(0) == "res://b.tscn", "tscn subgroup holds tscn files")
	d.free()


func _r_dirs_tree(h) -> void:
	var dirs := [
		{"path": "res://b", "created": 0, "modified": 2000, "files": 1, "subdirs": 0, "files_recursive": 1, "subdirs_recursive": 0, "size": 100, "size_recursive": 100},
		{"path": "res://a", "created": 0, "modified": 3000, "files": 2, "subdirs": 1, "files_recursive": 3, "subdirs_recursive": 1, "size": 300, "size_recursive": 400},
		{"path": "res://addons/z", "created": 0, "modified": 1000, "files": 0, "subdirs": 0, "files_recursive": 0, "subdirs_recursive": 0, "size": 50, "size_recursive": 50},
	]
	h.check(_paths(Dock.sort_dir_entries(dirs, "path", false)) == ["res://a", "res://addons/z", "res://b"], "dirs sort path")
	h.check(_paths(Dock.sort_dir_entries(dirs, "size", false)) == ["res://addons/z", "res://b", "res://a"], "dirs sort size")
	h.check(_paths(Dock.sort_dir_entries(dirs, "files", false)) == ["res://addons/z", "res://b", "res://a"], "dirs sort direct files")
	h.check(_paths(Dock.sort_dir_entries(dirs, "subdirs_recursive", true)) == ["res://a", "res://b", "res://addons/z"], "dirs sort recursive subdirs desc")
	h.check(_paths(Dock.sort_dir_entries(dirs, "size_recursive", false)) == ["res://addons/z", "res://b", "res://a"], "dirs sort recursive size")
	h.check(_paths(Dock.sort_dir_entries(dirs, "modified", true)) == ["res://a", "res://b", "res://addons/z"], "dirs sort modified desc")
	h.check(_paths(Dock.sort_dir_entries(dirs, "bogus", false)) == ["res://a", "res://addons/z", "res://b"], "dirs sort unknown falls back to path")
	h.check(Dock.sort_dir_entries(["junk", {"path": ""}], "path", false).is_empty(), "dirs sort drops bad rows")
	var census := {
		"extensions": {"gd": 3}, "total": 3,
		"project": {"extensions": {"gd": 2}, "total": 2},
		"addons": {"extensions": {"gd": 1}, "total": 1},
		"files": [{"path": "res://a.gd", "size": 10, "created": 0, "modified": 1000}],
		"dirs": dirs,
	}
	var d := _new_dock()
	d.call("set_file_results", "res://a.gd", [])
	d.call("set_census", census)
	var dtree: Tree = (d as Object).get("_dirs_tree")
	h.check(dtree.get_root().get_child_count() == 3, "dirs tree lists every dir")
	h.check(dtree.get_root().get_child(0).get_text(0) == "res://a", "dirs default sort path")
	var row := dtree.get_root().get_child(0)
	h.check(row.get_text(1) == "2", "dirs direct files")
	h.check(row.get_text(2) == "1", "dirs direct subdirs")
	h.check(row.get_text(3) == "3", "dirs recursive files")
	h.check(row.get_text(4) == "1", "dirs recursive subdirs")
	h.check(row.get_text(5) == "300 B", "dirs direct size")
	h.check(row.get_text(6) == "400 B", "dirs recursive size")
	_goto_seen.clear()
	row.select(0)
	h.check(_goto_seen.is_empty(), "dirs row inert")
	var ftree: Tree = (d as Object).get("_files_tree")
	_flip_panel(d, Dock.PANEL_DIRS, "sort", 1)
	h.check(dtree.get_root().get_child(0).get_text(0) == "res://addons/z", "dirs sort dropdown re-sorts")
	h.check(str(_panel_state(d, Dock.PANEL_DIRS).get("sort", "")) == "files", "dirs dropdown stores count key")
	h.check((_panel_widget(d, Dock.PANEL_DIRS, "sort") as OptionButton).item_count == 9, "dirs dropdown lists every column")
	h.check((_panel_widget(d, Dock.PANEL_FILES, "sort") as OptionButton).item_count == 4, "files dropdown lists four keys")
	h.check(ftree.get_root().get_child(0).get_child(0).get_child(0).get_text(0) == "res://a.gd", "dirs sort spares files tree")
	_flip_panel(d, Dock.PANEL_DIRS, "desc", 1)
	h.check(dtree.get_root().get_child(0).get_text(0) == "res://a", "dirs desc toggle inverts")
	_flip_panel(d, Dock.PANEL_DIRS, "addons", 0)
	h.check(dtree.get_root().get_child_count() == 2, "dirs addons toggle narrows")
	h.check(ftree.get_root().get_child_count() == 2, "dirs toggle spares files groups")
	_flip_panel(d, Dock.PANEL_DIRS, "addons", 1)
	d.call("set_census", {})
	h.check(dtree.get_root().get_child_count() == 1, "dirs hint when uncensused")
	h.check(dtree.get_root().get_child(0).get_text(0) == Dock.CENSUS_HINT, "dirs hint text")
	d.free()
...
func _r_column_collapse(h) -> void:
	...
	var tree: Tree = (d as Object).get("_files_tree")
	var dtree: Tree = (d as Object).get("_dirs_tree")
	h.check(Dock.column_sort_key(Dock.PANEL_FILES, 0) == "path", "files path column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_FILES, 1) == "size", "files size column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_FILES, 2) == "created", "files created column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_FILES, 3) == "modified", "files modified column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_FILES, 9) == "", "files overflow sorts nothing")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 0) == "path", "dirs path column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 1) == "files", "dirs files column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 2) == "subdirs", "dirs subdirs column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 3) == "files_recursive", "dirs files rec column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 4) == "subdirs_recursive", "dirs subdirs rec column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 5) == "size", "dirs size column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 6) == "size_recursive", "dirs size rec column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 7) == "created", "dirs created column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 8) == "modified", "dirs modified column sorts")
	h.check(Dock.column_sort_key(Dock.PANEL_DIRS, 9) == "", "dirs overflow sorts nothing")
	h.check(Dock.column_sort_key("bogus", 0) == "", "unknown panel sorts nothing")
	h.check(not tree.is_column_expanding(1) and not dtree.is_column_expanding(5), "columns start autofitted")
	h.check(tree.is_column_expanding(0) and dtree.is_column_expanding(0), "path starts stretched")
	h.check(dtree.get_column_title(0) == "Path" + Dock.SORT_ASC_MARK, "dirs sort column marked")
	h.check(dtree.get_column_title(1) == "Files", "dirs unsorted column plain")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_RIGHT, tree, Dock.PANEL_FILES)
	h.check(not bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(1, true)), "right click records stretch")
	h.check(tree.is_column_expanding(1), "right click stretches tree column")
	h.check(not dtree.is_column_expanding(1), "right click spares sibling tree")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_RIGHT, tree, Dock.PANEL_FILES)
	h.check(bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(1, false)), "second right click records autofit")
	h.check(not tree.is_column_expanding(1), "second right click recompacts")
	d.call("_on_column_title_clicked", 0, MOUSE_BUTTON_MIDDLE, tree, Dock.PANEL_FILES)
	h.check(not bool(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).get(0, false)), "middle click ignored")
	d.call("_on_column_title_clicked", 99, MOUSE_BUTTON_RIGHT, tree, Dock.PANEL_FILES)
	h.check(((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).size() == 3, "out-of-range click ignored")
	d.call("_on_column_title_clicked", 5, MOUSE_BUTTON_RIGHT, tree, Dock.PANEL_FILES)
	h.check(not ((d as Object).get("_collapsed_cols").get(Dock.PANEL_FILES, {}) as Dictionary).has(5), "files table ignores dirs-only columns")
	d.call("_on_column_title_clicked", 5, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "path", "files table ignores dirs-only sort click")
	d.call("set_census", {"extensions": {"gd": 9}, "total": 9})
	var ftree2: Tree = (d as Object).get("_files_tree")
	h.check(ftree2.get_root().get_child_count() == 1, "legacy files rows build without bounds errors")
	h.check(ftree2.get_root().get_child(0).get_child_count() == 1, "legacy files extension rows")
	h.check(not ftree2.get_root().get_child(0).is_selectable(0), "legacy files group inert")
	h.check(not ftree2.get_root().get_child(0).get_child(0).is_selectable(0), "legacy files row inert")
	d.call("_set_row_selectable", ftree2.get_root().get_child(0), false, 9)
	h.check(not ftree2.get_root().get_child(0).is_selectable(0) and not ftree2.get_root().get_child(0).is_selectable(3), "oversized width clamps to table")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "size", "left click picks sort column")
	h.check(not bool(_panel_state(d, Dock.PANEL_FILES).get("descending", true)), "new sort column starts ascending")
	h.check((_panel_widget(d, Dock.PANEL_FILES, "sort") as OptionButton).selected == 1, "left click syncs sort dropdown")
	h.check(tree.get_column_title(1) == "Size" + Dock.SORT_ASC_MARK, "new sort column marked")
	h.check(tree.get_column_title(0) == "Path", "old sort column unmarked")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(bool(_panel_state(d, Dock.PANEL_FILES).get("descending", false)), "second left click flips order")
	h.check((_panel_widget(d, Dock.PANEL_FILES, "desc") as CheckButton).button_pressed, "second left click syncs order switch")
	h.check(tree.get_column_title(1) == "Size" + Dock.SORT_DESC_MARK, "flipped column marks descending")
	d.call("_on_column_title_clicked", 0, MOUSE_BUTTON_LEFT, tree, Dock.PANEL_FILES)
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "path", "left click switches sort column")
	h.check(not bool(_panel_state(d, Dock.PANEL_FILES).get("descending", true)), "switched column resets order")
	d.call("_on_column_title_clicked", 1, MOUSE_BUTTON_LEFT, dtree, Dock.PANEL_DIRS)
	h.check(str(_panel_state(d, Dock.PANEL_DIRS).get("sort", "")) == "files", "count column left click sorts")
	d.call("_on_column_title_clicked", 5, MOUSE_BUTTON_LEFT, dtree, Dock.PANEL_DIRS)
	h.check(str(_panel_state(d, Dock.PANEL_DIRS).get("sort", "")) == "size", "dirs size column left click sorts")
	h.check(str(_panel_state(d, Dock.PANEL_FILES).get("sort", "")) == "path", "dirs sort spares files panel")
	d.free()
	var d2 := _new_dock()
	...
	var tree2: Tree = (d2 as Object).get("_files_tree")
	...
	h.check(d2.call("_column_fit_width", tree2, 0) >= path_w, "fit covers hidden child paths")
	...
	h.check(d2.call("_column_fit_width", tree2, 1) >= maxi(size_w, size_title_w), "fit covers cells and title")
	h.check(d2.call("_column_fit_width", tree2, -1) == 0, "fit rejects negative")
	h.check(d2.call("_column_fit_width", tree2, 4) == 0, "fit rejects overflow")
	...
	d2.call("_on_column_title_clicked", 1, MOUSE_BUTTON_RIGHT, tree2, Dock.PANEL_FILES)
	d2.call("set_census", {
	...
	h.check(tree2.is_column_expanding(1), "rebuild keeps stretch")
	d2.free()
...
func _r_flat_view(h) -> void:
	...
	d.call("set_file_results", "res://a.gd", [{"severity": "error", "kind": "k", "message": "m", "line": 4, "column": 1, "path": "res://a.gd"}])
	d.call("set_census", census)
	var flat: Tree = (d as Object).get("_files_flat")
	var cols: Tree = (d as Object).get("_files_tree")
	var dflat: Tree = (d as Object).get("_dirs_flat")
	var dcols: Tree = (d as Object).get("_dirs_tree")
	h.check(flat.columns == 1, "flat single column")
	h.check(dflat.columns == 1, "dirs flat single column")
	...
	h.check(fproj.get_child_count() == 1, "flat ext subgroup")
	h.check(fproj.get_child(0).get_text(0) == ".gd", "flat subgroup dotted label")
	h.check(fproj.get_child(0).get_child_count() == 2, "flat project file rows")
	h.check(fproj.get_child(0).get_child(0).get_text(0) == Dock.flat_file_row(files[1]), "flat file row concatenated")
	h.check(not ("res://" in fproj.get_child(0).get_child(0).get_text(0)), "flat file row hides prefix")
	h.check(dflat.get_root().get_child_count() == dcols.get_root().get_child_count(), "dirs flat mirrors dirs columns")
	h.check(dflat.get_root().get_child(0).get_text(0) == Dock.flat_dir_row((census.get("dirs", []) as Array)[0]), "flat dir row concatenated")
	_goto_seen.clear()
	fproj.collapsed = false
	fproj.get_child(0).collapsed = false
	fproj.get_child(0).get_child(0).select(0)
	...
	h.check(vtabs.get_tab_title(1) == Dock.TAB_TEXT, "text view second")
	_flip_panel(d, Dock.PANEL_FILES, "sort", 1)
	h.check(flat.get_root().get_child(0).get_child(0).get_child(0).get_text(0) == Dock.flat_file_row(files[0]), "flat view follows files sort")
	_flip_panel(d, Dock.PANEL_FILES, "addons", 0)
	h.check(flat.get_root().get_child_count() == cols.get_root().get_child_count(), "flat mirrors files addons toggle")
	_flip_panel(d, Dock.PANEL_DIRS, "addons", 0)
	h.check(dflat.get_root().get_child_count() == 1, "dirs toggle spares plain dirs in flat")
	h.check(dcols.get_root().get_child_count() == 1, "dirs toggle spares plain dirs in columns")
	d.call("set_census", {"extensions": {"gd": 9}, "total": 9})
...
func _r_persist(h) -> void:
	...
	h.check(bool((back.get("files", {}) as Dictionary).get("include_addons", false)), "roundtrip defaults files addons on")
	h.check(str((back.get("dirs", {}) as Dictionary).get("sort", "")) == "path", "roundtrip defaults dirs sort")
	Dock.save_filters({"error": true, "warning": true, "note": true}, {"gd": true, "tscn": true, "tres": true, "godot": true, "other": true}, {"include_addons": false, "sort": "size", "descending": true}, {"include_addons": true, "sort": "modified", "descending": false})
	var back2 := Dock.load_filters()
	h.check(not bool((back2.get("files", {}) as Dictionary).get("include_addons", true)), "roundtrip keeps files addons off")
	h.check(str((back2.get("files", {}) as Dictionary).get("sort", "")) == "size", "roundtrip keeps files sort")
	h.check(bool((back2.get("files", {}) as Dictionary).get("descending", false)), "roundtrip keeps files order")
	h.check(str((back2.get("dirs", {}) as Dictionary).get("sort", "")) == "modified", "roundtrip keeps dirs sort")
